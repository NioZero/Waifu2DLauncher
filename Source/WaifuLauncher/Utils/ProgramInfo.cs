...
internal static class ProgramInfo
{
    ...
    private static Assembly CurrentAssembly { get => _currentAssembly ??= Assembly.GetExecutingAssembly(); }
    ...
    public static string AssemblyVersion
    {
        get
        {
            ...
            if (version.EndsWith(".0.0")) return version.Substring(0, version.LastIndexOf(".0.0", StringComparison.Ordinal));
            if (version.EndsWith(".0")) return version.Substring(0, version.LastIndexOf(".0", StringComparison.Ordinal));
            return version;
        }
    }
